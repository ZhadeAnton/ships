...
for (i = 0; i < acc.length; i++) {
  acc[i].addEventListener("click", function (e) {
    let element = $(e.currentTarget).closest('.questions__item').find('.questions__item-info');

    if (element.find('accordion').get() !== [] || element.find('menu-btn__plus').get() !== []) {
      acc.forEach(function (item) {
        $(item).closest('.questions__item').find('.questions__item-info').removeClass('active');
...
new Swiper('.swiper', (_Swiper = {
  ...
  speed: 1500,
  centeredSlides: true
}, _defineProperty(_Swiper, "centeredSlides", true), _defineProperty(_Swiper, "spaceBetween", 20), _defineProperty(_Swiper, "loop", true), _defineProperty(_Swiper, "breakpoints", {
  701: {
    slidesPerView: 3
  }
}), _Swiper));
